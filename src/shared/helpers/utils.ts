...
export class Utils {
  ...
  static rmFileAndEmptyFolder(path: string) {
    const directory = dirname(path)
    const checkUploadFolder = global.UPLOAD_PATH
    if (existsSync(path)) {
      rmSync(path)
    }
    if (trimEnd(checkUploadFolder, '/') !== trimEnd(directory, '/')) {
      Utils.rmFileAndEmptyFolder(directory)
    }
  }

  static getDomain(s: string) {
    const firstTLDs =
